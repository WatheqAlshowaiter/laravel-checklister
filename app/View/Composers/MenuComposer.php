...
class MenuComposer
{
    ...
    {
        $menu = \App\Models\ChecklistGroup::with(['checklists' =>
        function ($query) {
        ...
        if (is_null($last_action_at)) {
            $last_action_at = Carbon::now()->subYear(10);
        }
        ...
        foreach ($menu as $group) {
            ...
            foreach ($group['checklists'] as &$checklist) {
                ...
                $checklist['is_updated'] = !($group['is_new']) && !$checklist['is_new'] && Carbon::create($checklist['updated_at'])->greaterThan($last_action_at);
                $checklist['task'] = 1;
